...
int Sentence::compareMeanings(Meaning& m1, Meaning& m2)
{
    ...
    if (!(partOfSpeech1.compare("verb")||partOfSpeech2.compare("subst")))
        return verbVSsubstAdj(m1, m2);

    if (!(partOfSpeech1.compare("verb")||partOfSpeech2.compare("adj")))
        return verbVSsubstAdj(m1, m2);

    if (!(partOfSpeech1.compare("verb")||partOfSpeech2.compare("ppas")))
        return verbVSsubstAdj(m1, m2);

    if (!(partOfSpeech1.compare("verb")||partOfSpeech2.compare("pact")))
        return verbVSsubstAdj(m1, m2);
...
//rule for substantive/adjective following verb
//return 3 - meanings are matching; return less than 3 - not matching;
int Sentence::verbVSsubstAdj(Meaning& m1, Meaning& m2)
{
    int matching=2;
